...
{
	/* Not tested due to OS difference*/
	/*
	int index = 0;
	char ch;
	#ifdef _WIN32
	...
	#endif
	*/
	strncpy(password,"Password",USER_PASS_MAXLEN);
}
...
void route_admin(p_user_t* user_list, p_stu_t* stu_list, char* str1, char* str2)
{
	...
	while (1) {
		ui_cls();
		ui_admin();
		button = ask_for_input("Please enter a number:",true,1,9);
		switch (button) {
...
void route_guest(p_stu_t stu_list)
{
	...
	while (1)
	{
		ui_cls();
		ui_guest();
		button = ask_for_input("Please enter a number:",true,1,2);
		switch (button){
